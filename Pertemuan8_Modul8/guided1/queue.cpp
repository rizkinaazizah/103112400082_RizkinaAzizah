...
void createQueue(queue &Q){
    Q.head =  Q.tail = Nil; //2 2nya kosong
}
...
void enQueue(queue &Q, address nodeBaru){//insert ----dari tail
    if(isEmpty(Q) == true){
...
void deQueue(queue &Q){//delete yang dihapus dari paling depan ---dari head
    address nodeHapus;
    if(isEmpty(Q) == true){
...
void updateQueue(queue &Q, int posisi){
    if(isEmpty(Q) == true){
        ...
        if(posisi == 0){
            ...
            int count = 1;//sebagai iterasi
            bool found = false;//penanda yang kita cari
            while(nodeBantu != Nil){
                if(count < posisi){
